...
bool VisionTools::is_node_set = false;
tf::TransformListener* VisionTools::tf_listener;

using namespace std;



bool VisionTools::setNodeHandle(ros::NodeHandle* nh)
{

        if(VisionTools::is_node_set)
                return true;
        if(nh == 0)
                return false;
        cout << "\033[1;37m     VisionTools.->Setting ros node...\033[0m" << endl;
        tf_listener = new tf::TransformListener();
        tf_listener->waitForTransform("map", "base_range_sensor_link", ros::Time(0), ros::Duration(10.0));
        VisionTools::is_node_set = true;
        return true;
}

bool VisionTools::transformPoint(std::string src_frame, float inX, float inY, float inZ, std::string dest_frame, float& outX, float& outY, float& outZ)
{
        cout << "\033[1;37m     VisionTools.-> " << "Transform point : " << inX <<" " << inY << " " << inZ << endl;
        cout << "\033[1;37m     VisionTools.-> " << "In : " << outX <<" " << outY << " " << outZ << endl;
...
void VisionTools::PointCloud2Msg_ToCvMat(sensor_msgs::PointCloud2& pc_msg, cv::Mat& bgr_dest, cv::Mat& pc_dest)
{
        // #################################
        //      Transform using PCL
        // #################################
        ...
        if(!pc_pcl.isOrganized()){
                cout << "\033[1;37m     VisionTools.->Point cloud is not organized!! O M G!!!!\033[0m" << endl;
                return;
        }
...
void VisionTools::PointCloud2Msg_ToCvMat(const sensor_msgs::PointCloud2::ConstPtr& pc_msg, cv::Mat& bgr_dest, cv::Mat& pc_dest)
{
